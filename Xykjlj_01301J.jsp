...
<%
	//フォームデータ解析
	pageParameters = new Xykjlj_01301_Params(request);
	Xykjlj_01301_Params param = (Xykjlj_01301_Params)pageParameters;
	
	//テーブル作成
	Xykj0130Renewday renewdayTable = new Xykj0130Renewday(param);
	renewdayTable.setMinLines(1);

	//テーブル作成
	Xykj0130DetailTable detailTable = new Xykj0130DetailTable(param);
	detailTable.setMinLines(1);

	//テーブル作成 2003/12/18 miura: checkTable2削除
	Xykj0130CheckTable checkTable = new Xykj0130CheckTable(param);
...
// *---------------------------------------------------------------------
// * システム　　名称 : 出版物管理システム
// * 画面名称		  : 出版社 銘柄/発売日（全国）（Xykjlj_01301J.jsp）
...
function getKEYCODE(e){
	...
	// ****************
	function initial(){
		document.title = G_name( "Sxykj01301" );	// *** 画面名称取得 ***
	...
	//	 document.form1.elements[eno0].style.background='pink';
		...
		if(document.form1.CommandType.value == "<%= Xykjlj_01301_Params.COMMAND_REFERENCE %>"){
...
// 2003/12/18 kimura:ポップアップメッセージを削除
	...
	function download_chk(){
		...
		document.form1.CommandType.value = "<%= Xykjlj_01301_Params.COMMAND_DOWNLOAD %>";
		document.form1.method = "POST";
		//document.form1.action = "../xykj/Xykjlj_01301Jd";
		document.form1.action = "<%= XysfljGenericRules.DOWNLOAD_PATH_XYKJ%>Xykjlj_01301Jd";
...
// 2003/12/18 kimura:ポップアップメッセージを削除
	...
	function button1_func(){
		...
		document.form1.CommandType.value = "<%= Xykjlj_01301_Params.COMMAND_REFERENCE %>";
...
<FORM method="post" name="form1" target="_top" action="Xykjlj_01301J.jsp">
<%= param.createTagOfStatus()%>
<%= param.createTagOfCommandType()%>
<SCRIPT language="JavaScript">
<!--
	Header("Sxykj01301","",975,0,0,0,0,1);
//			|			|	| | | | |
//			|			|	| | | | +---> 日付時刻 SW
//			|			|	| | | +-----> HELP SW
//			|			|	| | +-------> 印刷 SW
...
<INPUT type ="hidden" name="screenId" value="SXYKJ01301" >
