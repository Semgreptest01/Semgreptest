...
<%
	//フォームデータ解析
	pageParameters = new Xykjlj_02201_Params(request);
	Xykjlj_02201_Params param = (Xykjlj_02201_Params)pageParameters;
	
	//テーブル作成
	Xykj0220Renewday renewdayTable = new Xykj0220Renewday(param);
	renewdayTable.setMinLines(1);

	//テーブル作成
	Xykj0220CheckTable checkTable = new Xykj0220CheckTable(param);
	checkTable.setMinLines(1);

	//テーブル作成
	Xykj0220DetailTable detailTable = new Xykj0220DetailTable(param);
...
// *---------------------------------------------------------------------
// * システム　　名称 : 出版物管理システム
// * 画面名称		  : DEPT-CLASS-SUBCLASS-発行形態　別　検索（Xykjlj_02201J.jsp）
// * 会社名or所属名   : ＣＶＳシステム事業部
// * 作成日 		  : 2003/11/20 11:00:00
// * 作成者 		  : A.Konishi
// * *** 修正履歴 ***
...
<!--
$help_path		   = "<%= XysfljGenericRules.CONT_PATH_XYKJ%>help/Xykjlj_02201.htm";
-->
</SCRIPT>
<SCRIPT language="JavaScript" src="<%= XysfljGenericRules.CONT_PATH_XYKJ%>common/common.js"></SCRIPT>
<SCRIPT language="JavaScript" src="<%= XysfljGenericRules.CONT_PATH_XYKJ%>common/news_publishcom_product.js"></SCRIPT>
...
//	$submit_sw = "0";			// 送信中 SWITCH OFF
	function dataNull(){
		var NewsCombo = document.form1; 
//		alert(NewsCombo.select_publishcom.value);
//		alert(NewsCombo.select_product.options.length);
		if (NewsCombo.select_publishcom.value !==null && NewsCombo.select_product.options.length < 2){
 			alert("データがありません");
			NewsCombo = null;
			return false;
		}
...
function getKEYCODE(e){
	...
	// ****************
	function initial(){
		document.title = G_name( "Sxykj02201" );	// *** 画面名称取得 ***
		resizeTo(1024,740);		// 画面初期表示リサイズ
		focus();				// 自画面へフォーカス
		self.moveTo(0,0);		// Window 位置を0,0へ移動
		if(document.form1.CommandType.value == "<%= Xykjlj_02201_Params.COMMAND_REFERENCE %>"){
		...
		if ( document.form1.select_publishcom.value != 0 &&
			...
			news_publishcom_product_search(1);
		}
	}
// --------------------------------------
// ダウンロード ボタンが押された場合
// --------------------------------------
	function download_chk(){
		...
		document.form1.CommandType.value = "<%= Xykjlj_02201_Params.COMMAND_DOWNLOAD %>";
		document.form1.method = "POST";
		//document.form1.action = "../xykj/Xykjlj_02201Jd";
		document.form1.action = "<%= XysfljGenericRules.DOWNLOAD_PATH_XYKJ%>Xykjlj_02201Jd";
...
// --------------------------------------
	function button1_func(){
		...
		document.form1.CommandType.value = "<%= Xykjlj_02201_Params.COMMAND_REFERENCE %>";
...
<FORM method="post" name="form1" target="_top" action="Xykjlj_02201J.jsp">
<%= param.createTagOfStatus()%>
<%= param.createTagOfCommandType()%>
<SCRIPT language="JavaScript">
<!--
	Header("Sxykj02201","",975,0,0,0,0,1);
//			|			|	| | | | |
//			|			|	| | | | +---> 日付時刻 SW
//			|			|	| | | +-----> HELP SW
//			|			|	| | +-------> 印刷 SW
...
<TBODY><TR>
	...
	<TD>新聞社</TD>
	<TD><%= (param.getControl(XykjljNewsPUBLISHCOMCombo.NAME)).createTag()%></TD>
	<TD></TD>
	<TD></TD>
	<TD></TD>
</TR><TR>
	...
	<TD></TD>
	<TD>商品</TD>
	<TD colspan="3"><%= (param.getControl(XykjljNewsPublishProductCombo.NAME)).createTag()%></TD>
	<TD>
...
<INPUT type ="hidden" name="screenId" value="SXYKJ02201" >
<INPUT type ="hidden" name="screenNm" value="220" >
</FORM>
</BODY>
</HTML>
