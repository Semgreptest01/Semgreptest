...
<%
	//フォームデータ解析
	pageParameters = new Xykjlj_02301_Params(request);
	Xykjlj_02301_Params param = (Xykjlj_02301_Params)pageParameters;
	
	//テーブル作成
	Xykj0230Renewday renewdayTable = new Xykj0230Renewday(param);
	renewdayTable.setMinLines(1);

	//テーブル作成
	Xykj0230DetailTable detailTable = new Xykj0230DetailTable(param);
	detailTable.setMinLines(1);

	//テーブル作成
	Xykj0230CheckTable checkTable = new Xykj0230CheckTable(param);
...
// *---------------------------------------------------------------------
// * システム　　名称 : 出版物管理システム
// * 画面名称		  : 出版社 銘柄/発売日（全国）（Xykjlj_02301J.jsp）
...
<!--
$help_path		   = "<%= XysfljGenericRules.CONT_PATH_XYKJ%>help/Xykjlj_02301.htm";
-->
</SCRIPT>
...
function getKEYCODE(e){
	...
	// ****************
	function initial(){
		document.title = G_name( "Sxykj02301" );	// *** 画面名称取得 ***
	...
	//	 document.form1.elements[eno0].style.background='pink';
		...
		if(document.form1.CommandType.value == "<%= Xykjlj_02301_Params.COMMAND_REFERENCE %>"){
...
// --------------------------------------
	function download_chk(){
		...
		document.form1.CommandType.value = "<%= Xykjlj_02301_Params.COMMAND_DOWNLOAD %>";
		document.form1.method = "POST";
		//document.form1.action = "../xykj/Xykjlj_02301Jd";
		document.form1.action = "<%= XysfljGenericRules.DOWNLOAD_PATH_XYKJ%>Xykjlj_02301Jd";
...
// --------------------------------------
	function button1_func(){
		...
		document.form1.CommandType.value = "<%= Xykjlj_02301_Params.COMMAND_REFERENCE %>";
...
<FORM method="post" name="form1" target="_top" action="Xykjlj_02301J.jsp">
<%= param.createTagOfStatus()%>
<%= param.createTagOfCommandType()%>
<SCRIPT language="JavaScript">
<!--
	Header("Sxykj02301","",975,0,0,0,0,1);
//			|			|	| | | | |
//			|			|	| | | | +---> 日付時刻 SW
//			|			|	| | | +-----> HELP SW
//			|			|	| | +-------> 印刷 SW
...
<INPUT type ="hidden" name="screenId" value="SXYKJ02301" >
