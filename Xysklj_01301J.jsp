...
<%
	//フォームデータ解析
	pageParameters = new Xysklj_01301_Params(request);
	Xysklj_01301_Params param = (Xysklj_01301_Params)pageParameters;
	
	//テーブル作成
	Xysk0130Renewday renewdayTable = new Xysk0130Renewday(param);
	renewdayTable.setMinLines(1);

	//テーブル作成
	Xysk0130DetailTable detailTable = new Xysk0130DetailTable(param);
	detailTable.setMinLines(1);

	//テーブル作成 2003/12/18 miura: checkTable2削除
	Xysk0130CheckTable checkTable = new Xysk0130CheckTable(param);
...
// *---------------------------------------------------------------------
// * システム　　名称 : 出版物管理システム
// * 画面名称		  : 出版社 銘柄/発売日（全国）（Xysklj_01301J.jsp）
...
// * 02   2003/12/18  M.Kimura コンボボックスの連動制御・ユーザー制御
...
<!--
$help_path		   = "<%= XysfljGenericRules.CONT_PATH_XYSK%>help/Xysklj_01301.htmS";
-->
</SCRIPT>
...
function getKEYCODE(e){
	...
	// ****************
	function initial(){
		document.title = G_name( "Sxysk01301" );	// *** 画面名称取得 ***
	...
	//	 document.form1.elements[eno0].style.background='pink';
		...
		if(document.form1.CommandType.value == "<%= Xysklj_01301_Params.COMMAND_REFERENCE %>"){
		...
		// 2003/12/18 kimura:検索ボタン・ダウンロードボタンの制御追加
		button_sts_chg();
...
// ダウンロード ボタンが押された場合
// --------------------------------------
// 2003/12/18 kimura:ポップアップメッセージを削除
	...
	function download_chk(){
		...
		document.form1.CommandType.value = "<%= Xysklj_01301_Params.COMMAND_DOWNLOAD %>";
		document.form1.method = "POST";
		//document.form1.action = "../Xysk/Xysklj_01301Jd";
		document.form1.action = "<%= XysfljGenericRules.DOWNLOAD_PATH_XYSK%>Xysklj_01301Jd";
...
// 2003/12/18 kimura:ポップアップメッセージを削除
	...
	function button1_func(){
		...
		document.form1.CommandType.value = "<%= Xysklj_01301_Params.COMMAND_REFERENCE %>";
...
<FORM method="post" name="form1" target="_top" action="Xysklj_01301J.jsp">
<%= param.createTagOfStatus()%>
<%= param.createTagOfCommandType()%>
<SCRIPT language="JavaScript">
<!--
	Header("Sxysk01301","",975,0,0,0,0,1);
//			|			|	| | | | |
//			|			|	| | | | +---> 日付時刻 SW
//			|			|	| | | +-----> HELP SW
//			|			|	| | +-------> 印刷 SW
...
<INPUT type ="hidden" name="screenId" value="SXYSK01301" >
<INPUT type ="hidden" name="screenNm" value="130" >
</FORM>
</BODY>
</HTML>
