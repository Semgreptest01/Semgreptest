 ...
 *  L002  2006/11/15 S.Sawada   発売日２重表示対応
 *
 */
package xysk;
 ...
 * (ダウンロード)用クラス
 */
public class Xysklj_01301Jd extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse res)
		...
		//フォームデータ解析
		Xysklj_01301_Params param = null;

		try {
			param = new Xysklj_01301_Params(req);
			...
			if (csvheaddata != null) {
//L002 CHGS
//				headintsize = 4 + detailTable.maxday;
...
//L002 CHGS
//				csvintsize = 4 + detailTable.maxday;
...
// 2006/05/12変更(E10K移行対応)\
		// ***** ﾛｸﾞ出力処理 START *****************************************************************************
			param.putLog( 9,			 											// 状態（初期表示時:0 ﾀﾞｳﾝﾛｰﾄﾞ:9）
						...
						XysfljGenericRules.SCREEN_NM_230, 							// 画面名称
						XysfljGenericRules.SCREEN_NM_230);							// 画面MSG
		// ***** ﾛｸﾞ出力処理  END  ****************************************************************************

			// CSVファイルを出力
//			csvDownload(res, "Xysk1301.csv", csv1,csv2,csv3, getLineSeparator());
			csvDownload(res, "Xysk1301.csv", csv1,csv2,csv3, getLineSeparator(),detailTable.maxday);
...
//			response.setHeader("Content-Disposition","attachmentl; filename=" + fileName);
			...
			if (maxday != 0){
				...
				out.print(",,出版社　銘柄／発売日　別　販売実績" + kaigyo_cd );
			}
