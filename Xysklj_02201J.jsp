...
<%
	//フォームデータ解析
	pageParameters = new Xysklj_02201_Params(request);
	Xysklj_02201_Params param = (Xysklj_02201_Params)pageParameters;
	
	//テーブル作成
	Xysk0220Renewday renewdayTable = new Xysk0220Renewday(param);
	renewdayTable.setMinLines(1);

	//テーブル作成
	Xysk0220CheckTable checkTable = new Xysk0220CheckTable(param);
	checkTable.setMinLines(1);

	//テーブル作成
	Xysk0220DetailTable detailTable = new Xysk0220DetailTable(param);
...
// *---------------------------------------------------------------------
// * システム　　名称 : 出版物管理システム
// * 画面名称		  : DEPT-CLASS-SUBCLASS-発行形態　別　検索（Xysklj_02201J.jsp）
// * 会社名or所属名   : ＣＶＳシステム事業部
...
<!--
$help_path		   = "<%= XysfljGenericRules.CONT_PATH_XYSK%>help/Xysklj_02201.htm";
-->
</SCRIPT>
<SCRIPT language="JavaScript" src="<%= XysfljGenericRules.CONT_PATH_XYSK%>common/common.js"></SCRIPT>
<SCRIPT language="JavaScript" src="<%= XysfljGenericRules.CONT_PATH_XYSK%>common/news_publishcom_product.js"></SCRIPT>
...
function getKEYCODE(e){
	...
	// ****************
	function initial(){
		document.title = G_name( "Sxysk02201" );	// *** 画面名称取得 ***
		resizeTo(1024,740);		// 画面初期表示リサイズ
		focus();				// 自画面へフォーカス
		self.moveTo(0,0);		// Window 位置を0,0へ移動
		if(document.form1.CommandType.value == "<%= Xysklj_02201_Params.COMMAND_REFERENCE %>"){
...
// --------------------------------------
	function download_chk(){
		...
		document.form1.CommandType.value = "<%= Xysklj_02201_Params.COMMAND_DOWNLOAD %>";
		document.form1.method = "POST";
		//document.form1.action = "../Xysk/Xysklj_02201Jd";
		document.form1.action = "<%= XysfljGenericRules.DOWNLOAD_PATH_XYSK%>Xysklj_02201Jd";
...
// --------------------------------------
	function button1_func(){
		...
		document.form1.CommandType.value = "<%= Xysklj_02201_Params.COMMAND_REFERENCE %>";
...
<FORM method="post" name="form1" target="_top" action="Xysklj_02201J.jsp">
<%= param.createTagOfStatus()%>
<%= param.createTagOfCommandType()%>
<SCRIPT language="JavaScript">
<!--
	Header("Sxysk02201","",975,0,0,0,0,1);
//			|			|	| | | | |
//			|			|	| | | | +---> 日付時刻 SW
//			|			|	| | | +-----> HELP SW
//			|			|	| | +-------> 印刷 SW
...
<TBODY><TR>
	...
	<TD></TD>
	<TD>商品</TD>
	<TD colspan="3"><%= (param.getControl(XyskljNewsPublishProductCombo.NAME)).createTag()%></TD>
	<TD>
...
<INPUT type ="hidden" name="screenId" value="SXYSK02201" >
