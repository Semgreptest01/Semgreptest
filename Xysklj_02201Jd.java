 ...
 * (ダウンロード)用クラス
 */
public class Xysklj_02201Jd extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse res)
		...
		//フォームデータ解析
		Xysklj_02201_Params param = null;

		try {
			param = new Xysklj_02201_Params(req);
...
//				headintsize = 5 + detailTable.maxday;
				headintsize = 4 + param.printDays;
			}
	
			//データ明細
			Vector csvdata = detailTable.csvdata();
//			if ( detailTable.maxday != 0 ) 
			if ( detailTable.renewday != null && !detailTable.renewday.equals("") ){
				// CSV形式に編集(ヘッダー部)
...
// 2006/05/12変更(E10K移行対応)\
		// ***** ﾛｸﾞ出力処理 START *****************************************************************************
			param.putLog( 9,			 											// 状態（初期表示時:0 ﾀﾞｳﾝﾛｰﾄﾞ:9）
						...
						XysfljGenericRules.SCREEN_NM_220, 							// 画面名称
						XysfljGenericRules.SCREEN_NM_220);							// 画面MSG
		// ***** ﾛｸﾞ出力処理  END  ****************************************************************************

			// CSVファイルを出力
//			csvDownload(res, "Xysk2201.csv", csv1,csv2,csv3, getLineSeparator());
			csvDownload(res, "Xysk2201.csv", csv1,csv2,csv3, getLineSeparator(),detailTable.renewday);
	...
	*/
	public static boolean csvDownload(HttpServletResponse response,
		...
		try{																		// try
			...
			if ( renewday != null && !renewday.equals("") ){
				out.print(",,新聞社-発行形態実績" + kaigyo_cd );
			}
